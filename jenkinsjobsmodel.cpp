...
void JenkinsJobsModel::updateHeader()
{
   ...
   emit dataChanged(QAbstractItemModel::createIndex(0, 0, _rootItem->parent()),
                    QAbstractItemModel::createIndex(0, 2, _rootItem->parent()), QVector<int>() << Qt::DisplayRole);
   emit rootItemUpdated();
}

JenkinsJobsModel::JenkinsJobsModel(QObject *parent)
   : TreeModel(parent)
   , _rootItem(new JenkinsTreeItem(tr("Jenkins"), JenkinsTreeItem::Type::Root))
{
   updateHeader();
   rootItem()->appendChild(_rootItem);
   _animationTimer = new QTimer(this);
   _animationTimer->setSingleShot(false);
   connect(_animationTimer, &QTimer::timeout, [=]() { updateAnimationRecursively(_rootItem); });
...
void JenkinsJobsModel::updateAnimationRecursively(Utils::TreeItem *rootItem)
{
   ...
   rootItem->forAllChildren([this](Utils::TreeItem *item) { updateAnimationRecursively(item); });
   ...
   else
   {
      ...
      {
         setData(castedItem->index(), 1.0, static_cast<int>(JenkinsTreeItem::JobRoles::AnimationOpacity));
      }
   }
}
...
void JenkinsJobsModel::setJenkinsJobs(QList<JenkinsJob> jobs)
{
   resetJobs(jobs);
}
...
{
   QVector<JenkinsTreeItem *> castedItems;

   _rootItem->forAllChildren(
       [&castedItems](Utils::TreeItem *item) { castedItems.append(static_cast<JenkinsTreeItem *>(item)); });
