   ...
   , _restRequestBuilder(builder)
{
   ...
   connect(_view, &QTreeView::customContextMenuRequested, this, &JenkinsPane::onCustomContextMenuRequested);
   ...
   connect(_jenkinsViewComboBox, &JenkinsViewComboBox::currentViewChanged, this, [=]() { _model->resetJobs({}); });

   connect(_jenkinsViewComboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),
           [=](int) { emit currentViewChanged(); });
}
...
ViewInfo JenkinsPane::getSelectedView() const
{
   return _jenkinsViewComboBox->getSelectedView();
}
...
void JenkinsPane::onCustomContextMenuRequested(const QPoint &point)
{
   ...
   openInBrowserEntry->setIcon(QIcon(QLatin1String(JenkinsCI::Constants::OPEN_IN_BROWSER_ICON)));
   ...
   if (item->itemType() == JenkinsTreeItem::Type::Job)
   {
      ...
      buildHistoryEntry->setIcon(QIcon(QLatin1String(JenkinsCI::Constants::BUILD_HISTORY_ICON)));
      contextMenu->addAction(buildHistoryEntry);
      connect(buildHistoryEntry, &QAction::triggered, this, &JenkinsPane::requestBuildHistory);
   }
   else
   {
      const auto action = new QAction(tr("Force fetch"));
      contextMenu->addAction(action);
      connect(action, &QAction::triggered, this, &JenkinsPane::signalForceFetch);
   }
